...
class RL_Trainer(object):
    ...
    def dump_density_graphs(self, itr):
        ...
        if isinstance(self.agent, CBEAgent):
            ...
            exploration_values = np.rot90(exploration_values)
            exploration_values = np.log(exploration_values) # it's hard to see the numbers otherwise
            # breakpoint()
            plt.imshow(exploration_values)
            plt.colorbar()
            plt.title('Predicted Log-Exploration Value')
